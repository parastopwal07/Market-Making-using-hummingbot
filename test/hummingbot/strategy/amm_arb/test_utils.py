...
class AmmArbUtilsUnitTest(unittest.TestCase):
    ...
    async def _test_create_arb_proposals(self):
        market_info1 = MarketTradingPairTuple(
            MockConnector1(client_config_map=ClientConfigAdapter(ClientConfigMap())), trading_pair, base, quote
        )
        market_info2 = MarketTradingPairTuple(
            MockConnector2(client_config_map=ClientConfigAdapter(ClientConfigMap())), trading_pair, base, quote
        )
