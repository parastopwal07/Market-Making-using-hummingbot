...
class BitstampApiOrderBookDataSourceTests(IsolatedAsyncioWrapperTestCase):
    ...
    async def asyncSetUp(self) -> None:
        ...
        self.data_source = BitstampAPIOrderBookDataSource(
    ...
    def _order_diff_event(self):
        resp = {
            "data": {
                ...
                "bids": [["60362", "0.11602627"]],
                "asks": [["60341", "0.22347000"]],
            },
    ...
    async def test_get_new_order_book_successful(self, mock_api):
        url = web_utils.public_rest_url(
            path_url=CONSTANTS.ORDER_BOOK_URL.format(self.ex_trading_pair), domain=self.domain
        )
    ...
    async def test_listen_for_subscriptions_subscribes_to_trades_and_order_diffs(self, ws_connect_mock):
        ...
        expected_trade_subscription = {
            "data": {"channel": CONSTANTS.WS_PUBLIC_LIVE_TRADES.format(self.ex_trading_pair)},
            "event": "bts:subscribe",
        }
        self.assertEqual(expected_trade_subscription, sent_subscription_messages[0])
        expected_diff_subscription = {
            "data": {"channel": CONSTANTS.WS_PUBLIC_DIFF_ORDER_BOOK.format(self.ex_trading_pair)},
            "event": "bts:subscribe",
        }
    ...
    async def test_listen_for_subscriptions_subscribes_to_trades_and_order_diffs2(self, ws_connect_mock):
        ...
        self.mocking_assistant.add_websocket_aiohttp_message(
            websocket_mock=ws_connect_mock.return_value, message=json.dumps(reconnect_event)
        )
    ...
    async def test_listen_for_trades_logs_exception(self):
        incomplete_resp = {"data": {}, "channel": "live_trades_COINALPHAHBOT", "event": "trade"}
    ...
    async def test_listen_for_order_book_diffs_logs_exception(self):
        incomplete_resp = {"data": {}, "channel": "diff_order_book_COINALPHAHBOT", "event": "data"}
    ...
    @aioresponses()
    @patch(
        "hummingbot.connector.exchange.bitstamp.bitstamp_api_order_book_data_source"
