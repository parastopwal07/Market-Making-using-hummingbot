...
class CoinbaseAdvancedTradeUtilTestCases(IsolatedAsyncioWrapperTestCase):
    ...
    def test_public_rest_url(self):
        ...
        # Test custom domain
        self.assertEqual(public_rest_url("/test", domain="us"), CONSTANTS.REST_URL.format(domain="us") + "/test")

        # Test signin url endpoints
        for endpoint in CONSTANTS.SIGNIN_ENDPOINTS:
            self.assertEqual(
                public_rest_url(endpoint), CONSTANTS.SIGNIN_URL.format(domain=CONSTANTS.DEFAULT_DOMAIN) + endpoint
            )
    ...
    def test_private_rest_url(self):
        ...
        # Test custom domain
        self.assertEqual(private_rest_url("/test", domain="us"), CONSTANTS.REST_URL.format(domain="us") + "/test")

        # Test signin url endpoints
        for endpoint in CONSTANTS.SIGNIN_ENDPOINTS:
            self.assertEqual(
                private_rest_url(endpoint), CONSTANTS.SIGNIN_URL.format(domain=CONSTANTS.DEFAULT_DOMAIN) + endpoint
            )
    ...
    @patch(
        "hummingbot.connector.exchange.coinbase_advanced_trade.coinbase_advanced_trade_web_utils" ".create_throttler",
        return_value=Mock(),
    )
    @patch(
    ...
    def test_build_api_factory(self, mock_get_current_server_time_s, mock_create_throttler, mock_init):
        ...
        mock_init.assert_called_once_with(
            throttler=mock_create_throttler.return_value, auth=None, rest_pre_processors=[ANY]
        )
    ...
    def test_get_timestamp_from_exchange_time(self):
        ...
        self.assertEqual(
            get_timestamp_from_exchange_time("2023-05-11T12:34:56.789012+00:01", "s"), expected_seconds - 60
        )
        self.assertEqual(
            get_timestamp_from_exchange_time("2023-05-11T12:34:56.789012-01:00", "s"), expected_seconds + 3600
        )
        ...
        self.assertEqual(
            get_timestamp_from_exchange_time("2023-05-11T12:34:56.789012+00:00", "seconds"), expected_seconds
        )
        self.assertEqual(
            get_timestamp_from_exchange_time("2023-05-11T12:34:56.789012+00:00", "second"), expected_seconds
        )
        self.assertEqual(
            get_timestamp_from_exchange_time("2023-05-11T12:34:56.789012+00:00", "milliseconds"), expected_milliseconds
        )
        self.assertEqual(
            get_timestamp_from_exchange_time("2023-05-11T12:34:56.789012+00:00", "millisecond"), expected_milliseconds
        )
    ...
    @patch(
        "hummingbot.connector.exchange.coinbase_advanced_trade.coinbase_advanced_trade_web_utils" ".private_rest_url"
    )
    async def test_get_current_server_time_s(self, mock_private_rest_url, mock_api_factory):
        ...
        mock_rest_assistant.execute_request.return_value = {
            "iso": "2007-04-05T14:30Z",
            "epochSeconds": 1175783400,
            "epochMillis": 1175783400123,
        }
