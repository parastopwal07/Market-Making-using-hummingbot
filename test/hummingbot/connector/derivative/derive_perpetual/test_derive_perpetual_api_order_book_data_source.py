...
class DeriveAPIOrderBookDataSourceTests(IsolatedAsyncioWrapperTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.connector._set_trading_pair_symbol_map(bidict({f"{self.base_asset}-PERP": self.trading_pair}))
    ...
    async def test_listen_for_subscriptions_subscribes_to_trades_diffs_and_orderbooks(self, ws_connect_mock):
        ...
        expected_subscription_payload = {
            "channels": [f"trades.{self.ex_trading_pair.upper()}", f"orderbook.{self.ex_trading_pair.upper()}.1.100"]
        }
