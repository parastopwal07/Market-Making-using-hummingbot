...
class XEMMMultipleLevelsConfig(ControllerConfigBase):
    ...
    maker_connector: str = Field(
        default="mexc", client_data=ClientFieldData(prompt=lambda e: "Enter the maker connector: ", prompt_on_new=True)
    )
    ...
    taker_connector: str = Field(
        default="binance",
        client_data=ClientFieldData(prompt=lambda e: "Enter the taker connector: ", prompt_on_new=True),
    )
    ...
    max_executors_imbalance: int = Field(
        default=1,
        client_data=ClientFieldData(prompt=lambda e: "Enter the maximum executors imbalance: ", prompt_on_new=True),
    )
