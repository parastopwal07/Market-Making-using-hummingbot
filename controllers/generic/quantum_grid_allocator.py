...
class QGAConfig(ControllerConfigBase):
    ...
    min_spread_between_orders: Decimal = Field(
        default=Decimal("0.0001"), client_data=ClientFieldData(is_updatable=True)  # 0.01% between orders
    )
    grid_tp_multiplier: Decimal = Field(
        default=Decimal("0.0001"), client_data=ClientFieldData(is_updatable=True)  # 0.2% take profit
    )
    # Grid safety parameters
    limit_price_spread: Decimal = Field(
        default=Decimal("0.001"), client_data=ClientFieldData(is_updatable=True)  # 0.1% spread for limit price
    )
    activation_bounds: Decimal = Field(
        default=Decimal("0.0002"), client_data=ClientFieldData(is_updatable=True)  # Activation bounds for orders
    )
    bb_lenght: int = 100
