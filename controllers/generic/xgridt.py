...
class GridRange:
    ...
    def __init__(self, config: GridRangeConfig):
        ...
        self.max_orders_per_batch = (
            min(self.max_open_orders, config.max_orders_per_batch)
            if config.max_orders_per_batch
            else self.max_open_orders
        )
    ...
    def get_grid_levels_proposal_to_create(
        self, mid_price: Decimal, active_executors: List[ExecutorInfo], current_time: int
    ):
        ...
        active_executors_order_placed = [
            executor for executor in active_executors if executor.is_active and executor.is_trading is False
        ]
    ...
    def stop_loss_condition(self, active_executors: List[ExecutorInfo], current_time: int):
        ...
        active_executors_by_grid_range = [
            executor for executor in active_executors if executor.custom_info["level_id"] in grid_level_ids
        ]
    ...
    def stop_loss_by_price_limit(self, mid_price: Decimal, current_time: int):
        if (
            ...
            and mid_price < self.limit_price
            or self.side == TradeType.SELL
            and mid_price > self.limit_price
        ):
            self.active = False
            self.close_type = CloseType.STOP_LOSS
    ...
    def take_profit_condition(self, mid_price: Decimal, current_time: int):
        if (
            self.side == TradeType.BUY
            and mid_price > self.end_price
            or self.side == TradeType.SELL
            and mid_price < self.start_price
        ):
            self.active = False
...
class XGridTConfig(ControllerConfigBase):
    ...
    executor_activation_bounds: Decimal = Field(
    ...
    min_order_amount: Optional[Decimal] = Field(default=Decimal("1"), client_data=ClientFieldData(is_updatable=True))
...
class XGridT(ControllerBase):
    def __init__(self, config: XGridTConfig, *args, **kwargs):
        ...
        self.max_records = (
            max(
                config.ema_short, config.ema_medium, config.ema_long, config.donchian_channel_length, config.natr_length
            )
            + 500
        )
    ...
    async def update_processed_data(self):
        ...
        df.ta.donchian(
            lower_length=self.config.donchian_channel_length,
            upper_length=self.config.donchian_channel_length,
            append=True,
        )
    ...
    def _check_and_update_grid_ranges(self):
        ...
        if (
            self.config.grid_update_interval
            and self._last_grid_levels_update + self.config.grid_update_interval < self.market_data_provider.time()
        ):
            for grid_range in self.active_grid_ranges():
        ...
        if (
            len(self.get_allowed_grid_ranges_by_signal(1)) < self.config.max_ranges_by_signal
            and self.processed_data["signal"] == 1
        ):
            ...
            return
        grid_range = GridRangeConfig(
            id=f"R{len(self.grid_ranges)}",
            timestamp=self.market_data_provider.time(),
            ...
            side=trade_type,
            min_order_amount=max(self.config.min_order_amount, self.trading_rules.min_notional_size),
    ...
    def get_unrealized_pnl(self):
        return (
            sum([executor.net_pnl_quote for executor in self.active_executors(is_trading=True)])
    ...
    def to_format_status(self) -> List[str]:
        ...
        for grid_range in self.grid_ranges:
            status.append(
                f"Grid Range {grid_range.id} active: {grid_range.active} start price: {grid_range.start_price:.4f} end price: {grid_range.end_price:.4f} limit price: {grid_range.limit_price:.4f}"
            )
            for level in grid_range.grid_levels:
                executor_related = next(
                    (
                        executor
                        for executor in self.executors_info
                        if executor.custom_info["level_id"] == level.id and executor.is_active
                    ),
                    None,
                )
                if executor_related:
        ...
        status.append(
            f"LONG TARGETS: TP: {self.processed_data['TP_LONG']:.4f} SL: {self.processed_data['SL_LONG']:.4f} LIMIT: {self.processed_data['LIMIT_LONG']:.4f}"
        )
        status.append(
            f"SHORT TARGETS: TP: {self.processed_data['TP_SHORT']:.4f} SL: {self.processed_data['SL_SHORT']:.4f} LIMIT: {self.processed_data['LIMIT_SHORT']:.4f}"
        )
        return status
