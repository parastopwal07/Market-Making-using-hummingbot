...
class DManV3ControllerConfig(DirectionalTradingControllerConfigBase):
    ...
    interval: str = Field(
        default="30m",
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the candle interval (e.g., 1m, 5m, 1h, 1d): ", prompt_on_new=True
        ),
    )
    bb_length: int = Field(
        default=100,
        client_data=ClientFieldData(prompt=lambda mi: "Enter the Bollinger Bands length: ", prompt_on_new=True),
    )
    bb_std: float = Field(
        default=2.0,
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the Bollinger Bands standard deviation: ", prompt_on_new=False
        ),
    )
    ...
    dynamic_order_spread: bool = Field(
        default=None,
        client_data=ClientFieldData(
            prompt=lambda mi: "Do you want to make the spread dynamic? (Yes/No) ", prompt_on_new=True
        ),
    )
    dynamic_target: bool = Field(
        default=None,
        client_data=ClientFieldData(
            prompt=lambda mi: "Do you want to make the target dynamic? (Yes/No) ", prompt_on_new=True
        ),
    )
