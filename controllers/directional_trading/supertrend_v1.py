...
class SuperTrendConfig(DirectionalTradingControllerConfigBase):
    ...
    multiplier: float = Field(
        default=4.0,
        client_data=ClientFieldData(prompt=lambda mi: "Enter the supertrend multiplier: ", prompt_on_new=True),
    )
    percentage_threshold: float = Field(
        default=0.01,
        client_data=ClientFieldData(prompt=lambda mi: "Enter the percentage threshold: ", prompt_on_new=True),
    )
...
class SuperTrend(DirectionalTradingControllerBase):
    ...
    async def update_processed_data(self):
        ...
        df["percentage_distance"] = (
            abs(df["close"] - df[f"SUPERT_{self.config.length}_{self.config.multiplier}"]) / df["close"]
        )
