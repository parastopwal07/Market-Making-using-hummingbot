...
class EnhancedPMM(ScriptStrategyBase):
    ...
    def calculate_metrics(self, current_portfolio_value):
        ...
        if self.initial_portfolio_value > 0:
            return_rate = (current_portfolio_value - self.initial_portfolio_value) / self.initial_portfolio_value
            self.portfolio_returns.append(return_rate)

        if len(self.portfolio_returns) > 1:
            # Sharpe Ratio
            risk_free_rate = 0.005  # Monthly risk-free rate
            excess_returns = [r - risk_free_rate for r in self.portfolio_returns]
            ...
            # Calmar Ratio
            annualized_return = np.mean(self.portfolio_returns) * 12  # Assuming monthly returns
