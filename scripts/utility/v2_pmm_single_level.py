...
class PMMWithPositionExecutorConfig(StrategyV2ConfigBase):
    ...
    order_amount_quote: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the amount of quote asset to be used per order (e.g. 30): ", prompt_on_new=True
        ),
    )
    executor_refresh_time: int = Field(
        ...
        gt=0,
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the time in seconds to refresh the executor (e.g. 20): ", prompt_on_new=True
        ),
    )
    spread: Decimal = Field(
        default=Decimal("0.003"),
        gt=0,
        client_data=ClientFieldData(prompt=lambda mi: "Enter the spread (e.g. 0.003): ", prompt_on_new=True),
    )
    leverage: int = Field(
        default=20,
        gt=0,
    ...
    position_mode: PositionMode = Field(
        default="HEDGE",
        client_data=ClientFieldData(prompt=lambda mi: "Enter the position mode (HEDGE/ONEWAY): ", prompt_on_new=True),
    )
    ...
    take_profit: Decimal = Field(
        ...
        gt=0,
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the take profit (as a decimal, e.g., 0.01 for 1%): ", prompt_on_new=True
        ),
    )
    time_limit: int = Field(
        ...
        gt=0,
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the time limit in seconds (e.g., 2700 for 45 minutes): ", prompt_on_new=True
        ),
    )
    take_profit_order_type: OrderType = Field(
        default="LIMIT",
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the order type for taking profit (LIMIT/MARKET): ", prompt_on_new=True
        ),
    )
