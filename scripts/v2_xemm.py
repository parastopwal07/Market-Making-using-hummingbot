...
class V2XEMMConfig(StrategyV2ConfigBase):
    ...
    maker_connector: str = Field(
        default="kucoin",
        client_data=ClientFieldData(prompt=lambda e: "Enter the maker connector: ", prompt_on_new=True),
    )
    maker_trading_pair: str = Field(
        default="LBR-USDT",
        client_data=ClientFieldData(prompt=lambda e: "Enter the maker trading pair: ", prompt_on_new=True),
    )
    taker_connector: str = Field(
        default="okx", client_data=ClientFieldData(prompt=lambda e: "Enter the taker connector: ", prompt_on_new=True)
    )
    taker_trading_pair: str = Field(
        default="LBR-USDT",
        client_data=ClientFieldData(prompt=lambda e: "Enter the taker trading pair: ", prompt_on_new=True),
    )
    target_profitability: Decimal = Field(
        default=0.006,
        client_data=ClientFieldData(prompt=lambda e: "Enter the target profitability: ", prompt_on_new=True),
    )
    min_profitability: Decimal = Field(
        default=0.003,
        client_data=ClientFieldData(prompt=lambda e: "Enter the minimum profitability: ", prompt_on_new=True),
    )
    max_profitability: Decimal = Field(
        default=0.008,
        client_data=ClientFieldData(prompt=lambda e: "Enter the maximum profitability: ", prompt_on_new=True),
    )
    order_amount_quote: Decimal = Field(
        default=100,
        client_data=ClientFieldData(prompt=lambda e: "Enter the order amount in quote asset: ", prompt_on_new=True),
    )
...
class V2XEMM(StrategyV2Base):
    ...
    def init_markets(cls, config: V2XEMMConfig):
        cls.markets = {
            config.maker_connector: {config.maker_trading_pair},
            config.taker_connector: {config.taker_trading_pair},
        }
    ...
    def determine_executor_actions(self) -> List[ExecutorAction]:
        ...
        active_buy_executors = self.filter_executors(
            executors=all_executors, filter_func=lambda e: not e.is_done and e.config.maker_side == TradeType.BUY
        )
        active_sell_executors = self.filter_executors(
            executors=all_executors, filter_func=lambda e: not e.is_done and e.config.maker_side == TradeType.SELL
        )
