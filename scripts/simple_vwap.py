...
class VWAPConfig(BaseClientModel):
    ...
    connector_name: str = Field(
        "binance_paper_trade",
        client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Exchange where the bot will place orders"),
    )
    ...
    total_volume_quote: Decimal = Field(
        1000,
        client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Total amount to buy/sell (in quote asset)"),
    )
    price_spread: float = Field(
        0.001,
        client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Spread used to calculate the order price"),
    )
    ...
    order_delay_time: int = Field(
        10, client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Delay time between orders (in seconds)")
    )
