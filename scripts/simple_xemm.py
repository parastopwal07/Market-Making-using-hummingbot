...
class SimpleXEMMConfig(BaseClientModel):
    ...
    order_amount: Decimal = Field(
        0.1,
        client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Order amount (denominated in base asset)"),
    )
    ...
    min_spread_bps: Decimal = Field(
        0, client_data=ClientFieldData(prompt_on_new=False, prompt=lambda mi: "Minimum spread (in basis points)")
    )
    slippage_buffer_spread_bps: Decimal = Field(
        100, client_data=ClientFieldData(prompt_on_new=False, prompt=lambda mi: "Slippage buffer (in basis points)")
    )
    max_order_age: int = Field(
        120, client_data=ClientFieldData(prompt_on_new=False, prompt=lambda mi: "Max order age (in seconds)")
    )
...
class SimpleXEMM(ScriptStrategyBase):
    ...
    def place_sell_order(
        ...
        sell_price_with_slippage = sell_result.result_price * Decimal(
